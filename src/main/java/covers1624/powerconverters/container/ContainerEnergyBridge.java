...
public class ContainerEnergyBridge extends Container {
	...
	public void updateProgressBar(int var, int value) {
		if (var == 1000)
			_bridge.setIsInputLimited(value == 0 ? false : true);
		else if (var == 1001)
			_bridge.setEnergyScaled(value);
		else {
			int side = var / _numParams;
			int sidevar = var % _numParams;

			if (sidevar == 0)
				_bridge.getDataForSide(ForgeDirection.getOrientation(side)).voltageNameIndex = value;
			if (sidevar == 1)
				_bridge.getDataForSide(ForgeDirection.getOrientation(side)).isConsumer = (value != 0);
			if (sidevar == 2)
			...
			if (sidevar == 5)
				_bridge.getDataForSide(ForgeDirection.getOrientation(side)).outputRate = value;
		}
	}
